...
export function Modal({ open, children, title, rightAction, onClose }: IModalProps) {
  return (
    <Dialog.Root open={open} onOpenChange={onClose}>
      <Dialog.Portal>
        <Dialog.Overlay
          className={cn("fixed inset-0 z-50 bg-black/80 backdrop-blur-sm", "modal-overlay")}
        />
        ...
        >
          <header className="flex h-12 items-center justify-between text-gray-800">
            ...
            <div className="grid h-12 w-12 place-items-center">{rightAction}</div>
          </header>
